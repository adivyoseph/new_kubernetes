...
func TestParallelImagePullsNotHitLimitWithSameImage(t *testing.T) {
	ctx := context.Background()
	...
	pods := []*v1.Pod{}
	for i := 0; i < maxParallelImagePulls+2; i++ {
		// 7 Pods with 5 distinct image
		imageIndex := i % maxParallelImagePulls
		pods = append(pods, &v1.Pod{
			ObjectMeta: metav1.ObjectMeta{
				Name:            fmt.Sprintf("test_pod_%d", i),
				Namespace:       "test-ns",
	...
	// All 7 EnsureImageExists should result in runtime calls
	// MaxParallelImagePulls should not hit by same image
	for i := 0; i < maxParallelImagePulls+2; i++ {
		go func(pod *v1.Pod) {
			container := pod.Spec.Containers[0]
			_, _, err := puller.EnsureImageExists(ctx, nil, pod, container.Image, nil, nil, "", container.ImagePullPolicy)
