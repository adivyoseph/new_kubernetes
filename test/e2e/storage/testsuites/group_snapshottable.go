...
func (t *VolumeGroupSnapshottableTestSuite) GetTestSuiteInfo() storageframework.TestSuiteInfo {
	return t.tsInfo
}

func (t *VolumeGroupSnapshottableTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	labelKey := "group"
	...
	ginkgo.Describe("VolumeGroupSnapshottable", func() {
		...
		ginkgo.Context("", func() {
			...
			createPodsAndVolumes := func(ctx context.Context) {
				for i := 0; i < groupTest.numPods; i++ {
					...
					for j := 0; j < groupTest.numVolumes; j++ {
						volume := storageframework.CreateVolumeResource(ctx, driver, groupTest.config, pattern, t.GetTestSuiteInfo().SupportedSizeRange)
			...
			ginkgo.It("should create snapshots for multiple volumes in a pod", func(ctx context.Context) {
				...
				framework.Gomega().Expect(status).NotTo(gomega.BeNil())
				volumes := status.(map[string]interface{})["pvcVolumeSnapshotRefList"]
				framework.Gomega().Expect(volumes).NotTo(gomega.BeNil())
				volumeList := volumes.([]interface{})
				framework.Gomega().Expect(len(volumeList)).To(gomega.Equal(groupTest.numVolumes))
